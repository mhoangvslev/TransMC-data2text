...
class e2eDataset_sample(Dataset):
    def __init__(self, csv_file1, csv_file2, tokenizer, sample=0.3):
        """
        Args:
            csv_file (string): csv 파일의 경로
        """
        ...
        for _ in range(len(self.conditions)):
            ...
            k += 1
        ran_num = random.sample(sample_order, int(len(self.conditions)*sample)) # 샘플링 수
...
#     e2e_dataset = e2eDataset_sample(csv_file1='dataset/trainset.csv', csv_file2='dataset/devset.csv', tokenizer=my_model.tokenizer, sample=0.3)
